...
Поле `releaseVersion` має збільшуватися разом із публічним релізом, а
`generatedAt` — бути новішим за попередню генерацію тієї самої версії: застосунок
зберігає останній прийнятий стан і відхиляє rollback навіть із валідним старим
підписом. На чистому встановленні state-файла немає, тому перший чинний manifest
приймається.
...
Workflow-файли в `0.6.5.0` уже використовують тільки повні 40-символьні commit
